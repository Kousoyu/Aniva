"""环境模块测试 — Stimulus + StimulusEvent + Environment + LifeCore 集成."""
...
class TestStimulusEvent:
    """StimulusEvent（时序调度）单元测试."""

    def test_default_construction(self):
        stim = Stimulus(position=(0.0, 0.0, 0.0))
        e = StimulusEvent(stimulus=stim, start_step=0, duration_steps=100)
        assert e.stimulus is stim
        assert e.start_step == 0
        assert e.duration_steps == 100
        assert e.end_step == 100

    def test_custom_construction(self):
        stim = Stimulus(position=(0.5, 0.5, 0.5), intensity=0.5, radius=0.2)
        e = StimulusEvent(stimulus=stim, start_step=50, duration_steps=200)
        assert e.start_step == 50
        assert e.duration_steps == 200
        assert e.end_step == 250

    def test_duration_steps_must_be_positive(self):
        stim = Stimulus(position=(0.0, 0.0, 0.0))
        with pytest.raises(ValueError):
            StimulusEvent(stimulus=stim, start_step=0, duration_steps=0)
        with pytest.raises(ValueError):
            StimulusEvent(stimulus=stim, start_step=0, duration_steps=-5)

    def test_start_step_must_be_non_negative(self):
        stim = Stimulus(position=(0.0, 0.0, 0.0))
        with pytest.raises(ValueError):
            StimulusEvent(stimulus=stim, start_step=-1, duration_steps=100)

    def test_is_active_within_window(self):
        stim = Stimulus(position=(0.0, 0.0, 0.0))
        e = StimulusEvent(stimulus=stim, start_step=100, duration_steps=50)
        assert not e.is_active(0)
        assert not e.is_active(99)
        assert e.is_active(100)
        assert e.is_active(120)
        assert e.is_active(149)
        assert not e.is_active(150)
...
class TestEnvironment:
    ...
    def test_remove_invalid_index_does_nothing(self):
        env = Environment()
        stim = Stimulus(position=(0.0, 0.0, 0.0))
        env.add_event(StimulusEvent(stimulus=stim, start_step=0, duration_steps=100))
        env.remove_event(5)
        env.remove_event(-1)
        assert len(env.events) == 1
    ...
    def test_single_stimulus_influences_nearby_units(self):
        ...
        env.add_event(StimulusEvent(stimulus=stim, start_step=0, duration_steps=10))
        units = {
    ...
    def test_stimulus_outside_window_ignored(self):
        env = Environment()
        stim = Stimulus(position=(0.0, 0.0, 0.0))
        env.add_event(StimulusEvent(stimulus=stim, start_step=100, duration_steps=10))
        units = {0: type("U", (), {"position": (0.0, 0.0, 0.0)})()}
    ...
    def test_multiple_stimuli_aggregate(self):
        env = Environment()
        stim1 = Stimulus(position=(0.0, 0.0, 0.0), intensity=0.5, radius=1.0)
        stim2 = Stimulus(position=(0.0, 0.0, 0.0), intensity=0.3, radius=1.0)
        env.add_event(StimulusEvent(stimulus=stim1, start_step=0, duration_steps=10))
        env.add_event(StimulusEvent(stimulus=stim2, start_step=0, duration_steps=10))
        units = {0: type("U", (), {"position": (0.0, 0.0, 0.0)})()}
        result = env.compute_influences(units, step=5)
    ...
    def test_different_positions_affect_different_units(self):
        env = Environment()
        stim1 = Stimulus(position=(0.0, 0.0, 0.0), intensity=1.0, radius=0.3)
        stim2 = Stimulus(position=(0.8, 0.0, 0.0), intensity=1.0, radius=0.3)
        env.add_event(StimulusEvent(stimulus=stim1, start_step=0, duration_steps=10))
        env.add_event(StimulusEvent(stimulus=stim2, start_step=0, duration_steps=10))
        units = {
            0: type("U", (), {"position": (0.0, 0.0, 0.0)})(),
...
class TestLifeCoreWithEnvironment:
    ...
    def test_stimulus_effect_is_spatial(self):
        ...
        core = LifeCore(cfg)
        env = Environment()
        stim = Stimulus(position=(0.8, 0.0, 0.0), intensity=1.0, radius=0.3)
        env.add_event(StimulusEvent(stimulus=stim, start_step=0, duration_steps=10))
    ...
    def test_determinism_same_stimulus_same_result(self):
        ...
        def run_with_stimulus():
            ...
            core = LifeCore(cfg)
            env = Environment()
            stim = Stimulus(position=(0.0, 0.0, 0.0), intensity=0.5, radius=0.5)
            env.add_event(StimulusEvent(stimulus=stim, start_step=0, duration_steps=20))
    ...
    def test_stimulus_removal_leaves_trace(self):
        ...
        env = Environment()
        stim = Stimulus(position=(0.0, 0.0, 0.0), intensity=0.5, radius=0.5)
        env.add_event(StimulusEvent(stimulus=stim, start_step=50, duration_steps=100))
